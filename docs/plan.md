...
- **Source JDK path (Java 17)**: C:\Users\E949238\.jdk\jdk-17.0.16\bin
- **Target JDK path (Java 21)**: <span style="color:red"><to be installed, or specify the path here></span>
- **Build tool type**: Maven
- **Build tool path**: C:\Users\E949238\.maven\maven-3.9.12\bin
...
- Keep existing Spring Boot version and dependencies; only change Java version.
- Minimize code changes; prefer configuration/build updates over source refactors.
- Preserve current behavior; avoid introducing new features.
- Prefer automated, tool-driven changes over manual edits where possible.
