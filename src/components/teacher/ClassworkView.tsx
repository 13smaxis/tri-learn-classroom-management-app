...
interface LearnerRow {
  ...
  totalStars: number; classworkStars: number; classworkStarAwarded?: boolean; submissionId: string;
}
interface ClassworkDetail {
  classworkId: string; title: string; description?: string;
...
const ClassworkView: React.FC = () => {
  ...
  const [classworkList, setClassworkList] = useState<ClassworkItem[]>([]);
  const [loadingClasswork, setLoadingClasswork] = useState(false);
  const [refreshingClasswork, setRefreshingClasswork] = useState(false);

  const [selectedClasswork, setSelectedClasswork] = useState<ClassworkItem | null>(null);
  const [detail, setDetail] = useState<ClassworkDetail | null>(null);
  ...
  const [newClasswork, setNewClasswork] = useState({ title: '', description: '', dueDate: '', attachments: [] as File[], attachmentUrls: [] as string[] });
  ...
  const [classworkSuccess, setClassworkSuccess] = useState('');
  ...
  const canCreate = !!newClasswork.title.trim() && !!newClasswork.dueDate && !!selectedClass;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewClasswork(p => ({ ...p, attachments: fileArr }));
    const urls: string[] = [];
    const token = localStorage.getItem('authToken');
    for (const file of fileArr) {
      const fd = new FormData(); fd.append('attachment', file);
      try {
        const res = await fetch('/api/classwork/upload-attachment', { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : undefined, body: fd });
        const data = await res.json();
        if (data.success && data.url) urls.push(data.url);
      } catch { /* ignore */ }
    }
    setNewClasswork(p => ({ ...p, attachmentUrls: urls }));
  };

  /* ── Fetch classwork list for a class ── */
  const fetchClasswork = useCallback(async (classId: string, options?: { silent?: boolean }) => {
    const silent = !!options?.silent;
    setClassworkError('');
    setShowCreateForm(false);
    if (silent) {
      setRefreshingClasswork(true);
    } else {
      setLoadingClasswork(true);
    }

    try {
      const nextList = (await api.getClassworkList(classId)) || [];
      setClassworkList(nextList);

      if (selectedClasswork) {
        const nextSelected = nextList.find((item) => item.id === selectedClasswork.id) || null;
        if (nextSelected) {
          setSelectedClasswork(nextSelected);
        } else {
      ...
      setClassworkError(err instanceof Error ? err.message : 'Failed to load classwork');
    }
    finally {
      setLoadingClasswork(false);
      setRefreshingClasswork(false);
    }
  }, [selectedClasswork]);

  /* ── Fetch detail for a single classwork ── */
  const fetchDetail = useCallback(async (cw: ClassworkItem, options?: { silent?: boolean }) => {
    const silent = !!options?.silent;
    ...
    try {
      const nextDetail = await api.getClassworkDetail(cw.id);
      ...
      setClassworkError(err instanceof Error ? err.message : 'Failed to load detail');
    }
    finally {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await api.deleteClasswork(id);
      setClassworkList(l => l.filter(h => h.id !== id));
      if (selectedClasswork?.id === id) { setSelectedClasswork(null); setDetail(null); }
      setClassworkSuccess('Deleted.'); setTimeout(() => setClassworkSuccess(''), 2500);
    } catch (err) { setClassworkError(err instanceof Error ? err.message : 'Delete failed'); }
  };

  /* ── Create classwork (called after confirm) ── */
  const handleCreate = async () => {
    setShowConfirmDialog(false);
    setIsSaving(true);
    try {
      await api.createClasswork({
        classId: selectedClass, title: newClasswork.title, description: newClasswork.description,
        dueDate: normalizeDueDate(newClasswork.dueDate), attachmentUrls: newClasswork.attachmentUrls,
      });
      setIsSaving(false);
      setSuccessOverlay('Classwork assigned successfully!');
      setTimeout(() => {
        setSuccessOverlay('');
        setShowCreateForm(false);
        setNewClasswork({ title: '', description: '', dueDate: '', attachments: [], attachmentUrls: [] });
        fetchClasswork(selectedClass);
      }, 1800);
    } catch (err) {
      setIsSaving(false);
      setClassworkError(err instanceof Error ? err.message : 'Failed to create classwork');
    }
  };
  ...
  const handleSubmitLearnerRows = async () => {
    if (!selectedClasswork || !detail) return;
    ...
    for (const row of detail.learnerRows) {
      ...
      if (mark !== null && (isNaN(mark) || mark < 0 || mark > 100)) {
        setClassworkError(`Mark must be 0-100 for ${row.fullName}`);
        return;
      }
      if (!draft.submitted && mark !== null) {
        setClassworkError(`Cannot capture mark before marking ${row.fullName} as submitted`);
        return;
      }
    ...
    try {
      await api.submitClassworkEntries(selectedClasswork.id, entries);
      setClassworkSuccess('Classwork entries submitted successfully.');
      setTimeout(() => setClassworkSuccess(''), 2500);
      await fetchDetail(selectedClasswork, { silent: true });
    } catch (err) {
      setClassworkError(err instanceof Error ? err.message : 'Failed to submit classwork entries');
    } finally {
  ...
  const handleAwardStar = async (learnerId: string) => {
    ...
    try {
      await api.toggleClassworkStar(selectedClasswork.id, learnerId, selectedClasswork.classId);
      fetchDetail(selectedClasswork, { silent: true });
    }
    catch (err) { setClassworkError(err instanceof Error ? err.message : 'Star toggle failed'); }
  ...
  const placeholderMode = !showCreateForm && !selectedClasswork;
      ...
      <div className="flex flex-row h-full min-h-screen">
        ...
        <div className="w-full md:w-[30%] bg-blue-100/80 border-r border-blue-200 flex flex-col backdrop-blur-sm" style={{ maxHeight: '100vh' }}>
          ...
          <div className="p-5 pb-3 space-y-3 border-b border-blue-200/70">
            <h1 className="text-xl font-bold text-gray-900">Classwork</h1>
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => { setShowCreateForm(true); setSelectedClasswork(null); setDetail(null); setClassworkError(''); }}
            ...
            {classworkError && <p className="text-xs text-red-600 bg-red-50 rounded-lg px-3 py-1.5">{classworkError}</p>}
            {classworkSuccess && <p className="text-xs text-green-600 bg-green-50 rounded-lg px-3 py-1.5">{classworkSuccess}</p>}
            {refreshingClasswork && <p className="text-xs text-blue-700 bg-blue-100 rounded-lg px-3 py-1.5">Refreshing…</p>}
          </div>

          {/* Scrollable classwork list */}
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {!selectedClass ? (
              ...
              <div className="text-center py-12 text-gray-300 text-xs">No classwork assigned yet.</div>
            ) : (
              classworkList.map(cw => {
                const sel = selectedClasswork?.id === cw.id;
        ...
        <div ref={rightPanelRef} className="w-full md:w-[70%] overflow-y-auto" style={{ maxHeight: '100vh' }}>
            ...
            <div className="p-6 space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  { label: 'Classwork Submitted', value: '—', icon: '📝', bg: 'bg-blue-50' },
              ...
              <div className="bg-blue-50 rounded-xl border border-blue-200 shadow-sm p-5">
                <h3 className="text-sm font-semibold text-gray-400 mb-2">Classwork Description</h3>
              ...
              <div className="bg-blue-50 rounded-xl border border-blue-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6 text-center text-gray-300 text-sm">Select a class and click <b>View</b>, then choose a classwork to see results.</div>
              </div>
            </div>
          )}

          {/* ─────────────── CREATE CLASSWORK FORM ─────────────── */}
          {showCreateForm && (
              ...
              <div className="bg-blue-50 rounded-2xl border border-blue-200 p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Classwork Title <span className="text-red-400">*</span></label>
                    <input type="text" value={newClasswork.title}
                  ...
                  <div>
                    ...
                    <textarea value={newClasswork.description}
                      ...
                      placeholder="Enter classwork instructions…" />
                  </div>
                  <div className="space-y-5">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Due Date <span className="text-red-400">*</span></label>
                      <input type="datetime-local" value={newClasswork.dueDate}
                        onChange={e => setNewClasswork({ ...newClasswork, dueDate: e.target.value })}
                        className="w-full rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                    </div>
                    <div>
                      ...
                      {newClasswork.attachments.length > 0 && (
                        <ul className="mt-2 text-xs text-gray-500">{newClasswork.attachments.map((f, i) => <li key={i} className="flex items-center gap-1"><svg className="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.414a4 4 0 00-5.656-5.656l-6.415 6.414a6 6 0 108.486 8.486L20.5 13" /></svg>{f.name}</li>)}</ul>
                      )}
                    </div>
                  </div>
                  <div className="md:col-span-2 flex gap-3 pt-4 border-t border-blue-200/70">
                    <button type="button" onClick={() => { setShowCreateForm(false); setNewClasswork({ title: '', description: '', dueDate: '', attachments: [], attachmentUrls: [] }); }}
          ...
          {!showCreateForm && selectedClasswork && (
            <div className="p-6 animate-in fade-in duration-200">
              {loadingDetail ? (
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => { setSelectedClasswork(null); setDetail(null); }}
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      { v: `${detail.submittedCount} / ${detail.totalLearners}`, l: 'Classwork Submitted', icon: '📝', bg: 'bg-blue-50' },
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-blue-100/70 text-left text-[11px] text-gray-500 uppercase tracking-wider">
                            ...
                            <th className="px-4 py-2.5 text-center">Classwork Stars</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {detail.learnerRows.map((row, idx) => (
                            <tr key={row.learnerId} className="hover:bg-blue-100/60 transition-colors">
                              ...
                              <td className="px-4 py-2.5 text-center text-amber-500 font-semibold text-xs">⭐ {row.classworkStarAwarded ? 1 : 0}</td>
                              <td className="px-4 py-2.5 text-center">
                                <button onClick={() => handleAwardStar(row.learnerId)} disabled={awardingStarFor === row.learnerId}
                                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-medium transition disabled:opacity-50 ${row.classworkStarAwarded ? 'bg-rose-50 text-rose-700 hover:bg-rose-100' : 'bg-amber-50 text-amber-600 hover:bg-amber-100'}`}>
                                  {row.classworkStarAwarded ? '⭐ Remove' : '⭐ Award'}
                                </button>
                              </td>
                            </tr>
                          ))}
                ...
                <div className="text-center text-gray-400 py-20">Failed to load classwork details.</div>
              )}
            </div>
          )}
        </div>
          ...
          <div className="w-[min(92vw,420px)] rounded-2xl bg-white shadow-xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">You are about to assign this classwork to the selected class.</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
              <p><span className="text-gray-500">Title:</span> <span className="font-medium text-gray-900">{newClasswork.title}</span></p>
              <p><span className="text-gray-500">Due:</span> <span className="font-medium text-gray-900">{newClasswork.dueDate ? fmtDate(newClasswork.dueDate) : '—'}</span></p>
      ...
      {isSaving && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 backdrop-blur-sm">
          <div className="w-[min(92vw,360px)] rounded-2xl border border-border bg-card p-6 shadow-lg">
            ...
            <p className="mt-4 text-center text-sm font-medium text-foreground">Assigning classwork…</p>
          </div>
        </div>
      )}
