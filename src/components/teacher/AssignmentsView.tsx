...
interface LearnerRow {
  ...
  totalStars: number; assignmentStars: number; assignmentStarAwarded?: boolean; submissionId: string;
}
interface AssignmentDetail {
  assignmentId: string; title: string; description?: string;
...
const AssignmentsView: React.FC = () => {
  ...
  const [newAssignment, setNewAssignment] = useState({ title: '', description: '', dueDate: '', attachments: [] as File[], attachmentUrls: [] as string[] });
  ...
  const canCreate = !!newAssignment.title.trim() && !!newAssignment.dueDate && !!selectedClass;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewAssignment(p => ({ ...p, attachments: fileArr }));
    const urls: string[] = [];
    const token = localStorage.getItem('authToken');
    for (const file of fileArr) {
      const fd = new FormData(); fd.append('attachment', file);
      try {
        const res = await fetch('/api/assignment/upload-attachment', { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : undefined, body: fd });
        const data = await res.json();
        if (data.success && data.url) urls.push(data.url);
      } catch { /* ignore */ }
    }
    setNewAssignment(p => ({ ...p, attachmentUrls: urls }));
  };

  /* ── Fetch assignment list for a class ── */
  const fetchAssignment = useCallback(async (classId: string, options?: { silent?: boolean }) => {
    ...
    try {
      const nextList = (await api.getAssignmentList(classId)) || [];
      setAssignmentList(nextList);

      if (selectedAssignment) {
        const nextSelected = nextList.find((item) => item.id === selectedAssignment.id) || null;
        if (nextSelected) {
          setSelectedAssignment(nextSelected);
        } else {
  ...
  /* ── Fetch detail for a single assignment ── */
  const fetchDetail = useCallback(async (hw: AssignmentItem, options?: { silent?: boolean }) => {
    const silent = !!options?.silent;
    ...
    try {
      const nextDetail = await api.getAssignmentDetail(hw.id);
      ...
      setAssignmentError(err instanceof Error ? err.message : 'Failed to load detail');
    }
    finally {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      setAssignmentList(l => l.filter(h => h.id !== id));
      if (selectedAssignment?.id === id) { setSelectedAssignment(null); setDetail(null); }
      setAssignmentSuccess('Deleted.'); setTimeout(() => setAssignmentSuccess(''), 2500);
    } catch (err) { setAssignmentError(err instanceof Error ? err.message : 'Delete failed'); }
  };

  /* ── Create assignment (called after confirm) ── */
  const handleCreate = async () => {
    setShowConfirmDialog(false);
    setIsSaving(true);
    try {
      await api.createAssignment({
        classId: selectedClass, title: newAssignment.title, description: newAssignment.description,
        dueDate: normalizeDueDate(newAssignment.dueDate), attachmentUrls: newAssignment.attachmentUrls,
      });
      setIsSaving(false);
      setSuccessOverlay('Assignment assigned successfully!');
      setTimeout(() => {
        setSuccessOverlay('');
        setShowCreateForm(false);
        setNewAssignment({ title: '', description: '', dueDate: '', attachments: [], attachmentUrls: [] });
        fetchAssignment(selectedClass);
      }, 1800);
    } catch (err) {
      setIsSaving(false);
  ...
  const handleSubmitLearnerRows = async () => {
    if (!selectedAssignment || !detail) return;
    ...
    for (const row of detail.learnerRows) {
      ...
      if (mark !== null && (isNaN(mark) || mark < 0 || mark > 100)) {
        setAssignmentError(`Mark must be 0-100 for ${row.fullName}`);
        return;
      }
      if (!draft.submitted && mark !== null) {
        setAssignmentError(`Cannot capture mark before marking ${row.fullName} as submitted`);
        return;
      }
    ...
    try {
      await api.submitAssignmentEntries(selectedAssignment.id, entries);
      setAssignmentSuccess('Assignment entries submitted successfully.');
      setTimeout(() => setAssignmentSuccess(''), 2500);
      await fetchDetail(selectedAssignment, { silent: true });
    } catch (err) {
      setAssignmentError(err instanceof Error ? err.message : 'Failed to submit assignment entries');
    } finally {
  ...
  const handleAwardStar = async (learnerId: string) => {
    ...
    try {
      await api.toggleAssignmentStar(selectedAssignment.id, learnerId, selectedAssignment.classId);
      fetchDetail(selectedAssignment, { silent: true });
    }
    catch (err) { setAssignmentError(err instanceof Error ? err.message : 'Star toggle failed'); }
  ...
  const placeholderMode = !showCreateForm && !selectedAssignment;
    ...
    <div className="w-full h-full min-h-screen bg-transparent relative">
      <div className="flex flex-row h-full min-h-screen">
          ...
          <div className="p-5 pb-3 space-y-3 border-b border-blue-200/70">
              ...
              <select
                ...
                onChange={e => {
                  ...
                  if (nextClassId && viewRequested) {
                    fetchAssignment(nextClassId, { silent: true });
                  }
                }}
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => { setShowCreateForm(true); setSelectedAssignment(null); setDetail(null); setAssignmentError(''); }}
              ...
              <button
                disabled={!selectedClass}
                onClick={() => selectedClass && fetchAssignment(selectedClass, { silent: viewRequested })}
            ...
            {assignmentError && <p className="text-xs text-red-600 bg-red-50 rounded-lg px-3 py-1.5">{assignmentError}</p>}
            {assignmentSuccess && <p className="text-xs text-green-600 bg-green-50 rounded-lg px-3 py-1.5">{assignmentSuccess}</p>}
            {refreshingAssignment && <p className="text-xs text-blue-700 bg-blue-100 rounded-lg px-3 py-1.5">Refreshing…</p>}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {!viewRequested ? (
              <div className="text-center py-12 text-gray-300">
                ...
                <p className="text-xs">Select a class &amp; click <b>View</b></p>
              </div>
            ) : loadingAssignment ? (
              ...
              <div className="text-center py-12 text-gray-300 text-xs">No assignment assigned yet.</div>
            ) : (
              assignmentList.map(hw => {
                const sel = selectedAssignment?.id === hw.id;
                  ...
                  <div key={hw.id} onClick={() => fetchDetail(hw)}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        ...
                        {hw.description && <p className="text-[11px] text-gray-500 mt-0.5 line-clamp-1">{hw.description}</p>}
                        <div className="flex items-center gap-3 mt-1.5">
                          ...
                          {hw.attachmentUrls && hw.attachmentUrls.length > 0 && (
                            <span className="text-[11px] text-gray-300">{hw.attachmentUrls.length} file{hw.attachmentUrls.length > 1 ? 's' : ''}</span>
                          )}
                        </div>
                      </div>
                      <button onClick={e => handleDelete(hw.id, e)} className="p-1 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded transition" title="Delete">
        ...
        <div ref={rightPanelRef} className="w-full md:w-[70%] overflow-y-auto" style={{ maxHeight: '100vh' }}>
            ...
            <div className="p-6 space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  { label: 'Assignment Submitted', value: '—', icon: '📝', bg: 'bg-blue-50' },
              ...
              <div className="bg-blue-50 rounded-xl border border-blue-200 shadow-sm p-5">
                <h3 className="text-sm font-semibold text-gray-400 mb-2">Assignment Description</h3>
              ...
              <div className="bg-blue-50 rounded-xl border border-blue-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6 text-center text-gray-300 text-sm">Select a class and click <b>View</b>, then choose a assignment to see results.</div>
              </div>
            </div>
          )}
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      {newAssignment.attachments.length > 0 && (
                        <ul className="mt-2 text-xs text-gray-500">{newAssignment.attachments.map((f, i) => <li key={i} className="flex items-center gap-1"><svg className="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.414a4 4 0 00-5.656-5.656l-6.415 6.414a6 6 0 108.486 8.486L20.5 13" /></svg>{f.name}</li>)}</ul>
                      )}
                    </div>
                  </div>
                  <div className="md:col-span-2 flex gap-3 pt-4 border-t border-blue-200/70">
                    <button type="button" onClick={() => { setShowCreateForm(false); setNewAssignment({ title: '', description: '', dueDate: '', attachments: [], attachmentUrls: [] }); }}
          ...
          {!showCreateForm && selectedAssignment && (
            <div className="p-6 animate-in fade-in duration-200">
              {loadingDetail ? (
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => { setSelectedAssignment(null); setDetail(null); }}
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      { v: `${detail.submittedCount} / ${detail.totalLearners}`, l: 'Assignment Submitted', icon: '📝', bg: 'bg-blue-50' },
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-blue-100/70 text-left text-[11px] text-gray-500 uppercase tracking-wider">
                            ...
                            <th className="px-4 py-2.5 text-center">Assignment Stars</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {detail.learnerRows.map((row, idx) => (
                            <tr key={row.learnerId} className="hover:bg-blue-100/60 transition-colors">
                              ...
                              <td className="px-4 py-2.5 text-center text-amber-500 font-semibold text-xs">⭐ {row.assignmentStarAwarded ? 1 : 0}</td>
                              <td className="px-4 py-2.5 text-center">
                                <button onClick={() => handleAwardStar(row.learnerId)} disabled={awardingStarFor === row.learnerId}
                                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-medium transition disabled:opacity-50 ${row.assignmentStarAwarded ? 'bg-rose-50 text-rose-700 hover:bg-rose-100' : 'bg-amber-50 text-amber-600 hover:bg-amber-100'}`}>
                                  {row.assignmentStarAwarded ? '⭐ Remove' : '⭐ Award'}
                                </button>
                              </td>
                            </tr>
                          ))}
                ...
                <div className="text-center text-gray-400 py-20">Failed to load assignment details.</div>
              )}
            </div>
          )}
        </div>
          ...
          <div className="w-[min(92vw,420px)] rounded-2xl bg-white shadow-xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">You are about to assign this assignment to the selected class.</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
              <p><span className="text-gray-500">Title:</span> <span className="font-medium text-gray-900">{newAssignment.title}</span></p>
              <p><span className="text-gray-500">Due:</span> <span className="font-medium text-gray-900">{newAssignment.dueDate ? fmtDate(newAssignment.dueDate) : '—'}</span></p>
      ...
      {isSaving && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 backdrop-blur-sm">
          <div className="w-[min(92vw,360px)] rounded-2xl border border-border bg-card p-6 shadow-lg">
            ...
            <p className="mt-4 text-center text-sm font-medium text-foreground">Assigning assignment…</p>
          </div>
        </div>
      )}
