...
const HomeworkView: React.FC = () => {
  ...
  const [newHomework, setNewHomework] = useState({ title: '', description: '', dueDate: '', attachments: [] as File[], attachmentUrls: [] as string[] });
  ...
  const canCreate = !!newHomework.title.trim() && !!newHomework.dueDate && !!selectedClass;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewHomework(p => ({ ...p, attachments: fileArr }));
    const urls: string[] = [];
    const token = localStorage.getItem('authToken');
    for (const file of fileArr) {
      const fd = new FormData(); fd.append('attachment', file);
      try {
        const res = await fetch('/api/homework/upload-attachment', { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : undefined, body: fd });
        const data = await res.json();
        if (data.success && data.url) urls.push(data.url);
      } catch { /* ignore */ }
    }
    setNewHomework(p => ({ ...p, attachmentUrls: urls }));
  };
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      setHomeworkList(l => l.filter(h => h.id !== id));
      if (selectedHomework?.id === id) { setSelectedHomework(null); setDetail(null); }
      setHomeworkSuccess('Deleted.'); setTimeout(() => setHomeworkSuccess(''), 2500);
    } catch (err) { setHomeworkError(err instanceof Error ? err.message : 'Delete failed'); }
  };

  /* ── Create homework (called after confirm) ── */
  const handleCreate = async () => {
    setShowConfirmDialog(false);
    setIsSaving(true);
    try {
      await api.createHomework({
        classId: selectedClass, title: newHomework.title, description: newHomework.description,
        dueDate: normalizeDueDate(newHomework.dueDate), attachmentUrls: newHomework.attachmentUrls,
      });
      setIsSaving(false);
      setSuccessOverlay('Homework assigned successfully!');
      setTimeout(() => {
        setSuccessOverlay('');
        setShowCreateForm(false);
        setNewHomework({ title: '', description: '', dueDate: '', attachments: [], attachmentUrls: [] });
  ...
  const placeholderMode = !showCreateForm && !selectedHomework;
    ...
    <div className="w-full h-full min-h-screen bg-gray-50 relative">
      <div className="flex flex-row h-full min-h-screen">
        ...
        <div className="w-full md:w-[30%] bg-white border-r border-gray-200 flex flex-col" style={{ maxHeight: '100vh' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {!viewRequested ? (
              ...
              <div className="text-center py-12 text-gray-300 text-xs">No homework assigned yet.</div>
            ) : (
              homeworkList.map(hw => {
                const sel = selectedHomework?.id === hw.id;
        ...
        <div ref={rightPanelRef} className="w-full md:w-[70%] overflow-y-auto" style={{ maxHeight: '100vh' }}>
            ...
            <div className="p-6 space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  { label: 'Homework Submitted', value: '—', icon: '📝', bg: 'bg-blue-50' },
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      {newHomework.attachments.length > 0 && (
                        <ul className="mt-2 text-xs text-gray-500">{newHomework.attachments.map((f, i) => <li key={i} className="flex items-center gap-1"><svg className="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.414a4 4 0 00-5.656-5.656l-6.415 6.414a6 6 0 108.486 8.486L20.5 13" /></svg>{f.name}</li>)}</ul>
                      )}
                    </div>
                  </div>
          ...
          {!showCreateForm && selectedHomework && (
            <div className="p-6 animate-in fade-in duration-200">
              {loadingDetail ? (
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      { v: `${detail.submittedCount} / ${detail.totalLearners}`, l: 'Homework Submitted', icon: '📝', bg: 'bg-blue-50' },
          ...
          <div className="w-[min(92vw,420px)] rounded-2xl bg-white shadow-xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">You are about to assign this homework to the selected class.</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
              <p><span className="text-gray-500">Title:</span> <span className="font-medium text-gray-900">{newHomework.title}</span></p>
              <p><span className="text-gray-500">Due:</span> <span className="font-medium text-gray-900">{newHomework.dueDate ? fmtDate(newHomework.dueDate) : '—'}</span></p>
      ...
      {isSaving && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 backdrop-blur-sm">
          <div className="w-[min(92vw,360px)] rounded-2xl border border-border bg-card p-6 shadow-lg">
            ...
            <p className="mt-4 text-center text-sm font-medium text-foreground">Assigning homework…</p>
          </div>
        </div>
      )}
