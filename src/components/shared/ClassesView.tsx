...
const ClassesView: React.FC<ClassesViewProps> = ({ onCreateClass }) => {
  const { user } = useAuth();
  const [classes, setClasses] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedClass, setSelectedClass] = useState<any>(null);
  const [showInviteCodes, setShowInviteCodes] = useState<string | null>(null);
  ...
  const fetchClasses = async () => {
    ...
    try {
      if (user.role === 'teacher') {
        const { data } = await supabase.functions.invoke('edu-auth', {
          body: { action: 'getTeacherClasses', teacherId: user.id }
        });
        if (data?.classes) setClasses(data.classes);
      } else {
        const { data } = await supabase.functions.invoke('edu-auth', {
          body: { action: 'getUserClasses', userId: user.id, role: user.role }
        });
        if (data?.enrollments) {
          setClasses(data.enrollments.map((e: any) => e.classes));
        }
      }
    } catch (err) {
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  {user?.role !== 'teacher' && cls.users && (
                    <p className="text-sm text-gray-500 mb-4">
                      Teacher: {cls.users.full_name}
                    </p>
                  )}

                  {/* Invite Codes (Teacher only) */}
                  {user?.role === 'teacher' && showInviteCodes === cls.id && (
                    <div className="mb-4 space-y-3">
                      <div className="rounded-lg bg-purple-50 border border-purple-200 p-3">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium text-purple-700">Learner Code</span>
                          <button
                            onClick={() => copyToClipboard(cls.learner_invite_code)}
                            className="text-xs text-purple-600 hover:text-purple-700"
                          >
                            Copy
                          </button>
                        </div>
                        <p className="font-mono font-bold text-purple-900 mt-1">{cls.learner_invite_code}</p>
                      </div>
                      <div className="rounded-lg bg-green-50 border border-green-200 p-3">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium text-green-700">Parent Code</span>
                          <button
                            onClick={() => copyToClipboard(cls.parent_invite_code)}
                            className="text-xs text-green-600 hover:text-green-700"
                          >
                            Copy
                          </button>
                        </div>
                        <p className="font-mono font-bold text-green-900 mt-1">{cls.parent_invite_code}</p>
                      </div>
                    </div>
                  )}
